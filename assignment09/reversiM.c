...
bool legal(Game* g, int x, int y) {
    return !out_of_bounds(x, y) && ((g->board[x][y] == '_') || (g->board[x][y] == '*')) &&
...
void change_board(Game* g){
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < N; j++) {
            if(g->board[i][j] == '_' || g->board[i][j] == '*'){
                if(legal(g, i, j)){
                    char *p = &g->board[i][j];
                    *p = '*';
                }
...
Position computer_move(Game *g) {
    // todo: implement
    /*
        Zweite etwas mühselige Implementation!
    */
    PositionStack ps = make_position_stack();
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < N; j++) {
            if(legal(g, i, j)) {
                push(&ps, make_position(i,j));
            }
        }
    }
    if (ps.length <= 0) {
    ...
    /* erste und einfachste Möglichkeit , also Implementation ;)

    int upper_case_start = 65;
    int upper_case_end = 73;
    int numbers_start = 48;
    int numbers_end = 56;
    Position pos;
    while(true){
        char a = (rand() % (upper_case_end + 1 - upper_case_start)) + upper_case_start;
        char b = (rand() % (numbers_end + 1 - numbers_start)) + numbers_start;
        pos.x = (int)tolower(a) - 'a';
        pos.y = (int)tolower(b) - '1';
        if (legal(g, pos.x, pos.y)) {
            return pos;
        }
    }*/
